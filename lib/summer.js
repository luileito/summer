(function(window) {
  ...
  function Summer() {
    ...
    function processElementsFromRule(rule) {
      ...
      var values = ['--text-summary', '--text-summary-algorithm', '--text-summary-target', '--text-summary-list'];
    ...
    function searchInElement(elem, str) {
      ...
      if (window.find && window.getSelection) {
        ...
        // Old IE browsers only. Not tested!
        var textRange = elem.createTextRange();
        while (textRange.findText(str)) {
          textRange.execCommand('BackColor', false, 'white');
          ret = sel.anchorNode.parentNode.innerHTML;
          textRange.collapse(false);
        }
      }
      // Return search string as fallback.
      return ret || str;
    }
